 ...
 */
 ...
 typedef
 struct idt_entry_t {
     uint16_t offset_low;       // 中断处理函数偏移0~15位
     uint16_t selector;         // 目标代码段描述符选择子
     uint8_t always0;           // 置零段
     uint8_t flags;             // 其他的一些标志，见IDT表项结构
     uint16_t offset_high;      // 中断处理函数偏移 16~31位
...
extern idt_entry_t *idt_entries;
 ...
 struct pt_regs_t {
    // 按照入栈的顺序反序定义，ds为栈顶
     ...
     uint32_t eax;
     uint32_t int_no;       //中断号
     uint32_t err_code;     //错误代码，有中段错误代码的会自动压入（CPU自动）
     // 以下由处理器自动压入
     uint32_t eip;          
     uint32_t cs;
     uint32_t eflags;
