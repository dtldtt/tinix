...
// 表示当前块的最大值，永远是页对齐的，因为在alloc_block里边，每次+=PAGE_SiZE
...
{
    heap_first=0;
    heap_start = total_mem_bytes - 20*1024-1024;
    heap_max = heap_start;
}
...
void* kmalloc(uint32_t len)
{
    ...
    while(cur_header) {
        ...
        // 没有的话继续循环查找
        // 只要有任何一个内存块是已经分配过得（不是第一次访问这个函数）
        // 那么在退出这个函数的时候，prev_header就不会是0
        // 所以后边用prev_header是否为0来判断是不是第一次访问这个函数
        prev_header = cur_header;
        cur_header = cur_header->next;
    }

    // 如果第一次调用该函数或者没有找到合适的内存块，则需要申请内存块
    ...
    cur_header->next = 0;
    cur_header->allocated=1;
    cur_header->length=len; //内存块长度加上头部长度，函数开头已经加过了
    // 如果有前驱内存块的话，就让他指向现在这个
    if(prev_header) {
